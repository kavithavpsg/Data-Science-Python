...
company_data["Sales"] = company_data["Sales"].astype('category')
company_data["Sales_cat"] = company_data["Sales"].cat.codes
company_data.head()

company_data.loc[:,"Sales_cat"].mean()
company_data['Sales_cat'] = company_data.Sales_cat.apply(lambda x: 1 if x >= 165 else 0)
company_data["Sales_cat"] = company_data["Sales_cat"].astype('category')
company_data.dtypes

#data["default"]=lb.fit_transform(data["default"])

company_data['Sales_cat'].unique()
company_data['Sales_cat'].value_counts()
